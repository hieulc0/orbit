...
/// A coding Attempt is a normal, private Git repository.  The repository is
/// cloned with Git's isolation flags rather than by copying an arbitrary
/// `.git` directory from the worker checkout.  Orbit still owns the
/// authoritative Attempt state and only uses this repository for workspace
/// evidence and patch extraction.
...
impl Workspace {
    pub async fn materialize(
        ...
        let workspace = Self {
            ...
            git_dir: directory.join("repository").join(".git"),
            home: directory.join("git-home"),
        };
        tokio::fs::create_dir_all(&workspace.home).await?;

        // A worker restart must reclaim the same live Attempt rather than
        // replacing the agent's edits with a fresh checkout.  Only reuse a
        // repository that still proves the requested frozen baseline; a
        // mismatch fails closed.
        match tokio::fs::symlink_metadata(&workspace.git_dir).await {
            Ok(metadata) => {
                ensure!(
                    metadata.is_dir() && !metadata.file_type().is_symlink(),
                    "Attempt Git metadata is not a directory"
                );
                let revision = workspace.git(&["rev-parse", "HEAD"]).await?;
                    ...
                    "base revision mismatch while reclaiming Attempt workspace"
                );
                return Ok(workspace);
            }
        ...
        match tokio::fs::symlink_metadata(&workspace.path).await {
            Ok(metadata) => {
                ensure!(
                    metadata.is_dir() && !metadata.file_type().is_symlink(),
                    "Attempt repository is not a directory"
                );
                let mut entries = tokio::fs::read_dir(&workspace.path).await?;
                ensure!(
                    entries.next_entry().await?.is_none(),
                    "Attempt repository exists without trusted Git metadata"
                );
            }
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {}
            Err(error) => return Err(error.into()),
        ...
        // The coding Attempt needs local Git history, not a persistent remote.
        // Removing origin prevents a local source path or a credential-bearing
        // remote URL from becoming agent-visible repository configuration.
        workspace.git(&["remote", "remove", "origin"]).await?;
        workspace
            .git(&[
                "checkout",
                "--detach",
...
mod tests {
    ...
    fn git_fixture_command(directory: &Path, args: &[&str]) -> Result<Vec<u8>> {
    ...
    #[tokio::test]
    async fn materialized_attempt_is_a_reusable_isolated_git_repository() -> Result<()> {
        let temp = tempfile::tempdir()?;
        let source = temp.path().join("source");
        tokio::fs::create_dir(&source).await?;
        git_fixture_command(&source, &["init"])?;
        git_fixture_command(&source, &["config", "user.name", "fixture"])?;
        git_fixture_command(
            &source,
            &["config", "user.email", "fixture@example.invalid"],
        )?;
        tokio::fs::write(source.join("README.md"), b"baseline\n").await?;
        git_fixture_command(&source, &["add", "README.md"])?;
        git_fixture_command(&source, &["commit", "-m", "baseline"])?;
        let baseline = String::from_utf8(git_fixture_command(&source, &["rev-parse", "HEAD"])?)?
            .trim()
            .to_string();
        ...
        let plan = Plan {
            ...
            repository: RepositoryBinding {
                ...
                allowed_test_executables: vec!["true".into()],
                remote: None,
            },
            digest: "a".repeat(64),
        ...
        let first_root = temp.path().join("attempt-a");
        tokio::fs::create_dir(&first_root).await?;
        let first = Workspace::materialize(&assignment, &first_root, &credentials).await?;

        assert!(first.path.join(".git").is_dir());
        assert!(String::from_utf8(git_fixture_command(&first.path, &["remote"])?)?.is_empty());
        assert_eq!(
            String::from_utf8(git_fixture_command(&first.path, &["rev-parse", "HEAD"])?)?.trim(),
            baseline
        );
        assert!(
            String::from_utf8(git_fixture_command(&first.path, &["status", "--short"])?)?
                .is_empty()
        );
        tokio::fs::write(first.path.join("attempt-a.txt"), b"A\n").await?;

        // Re-materialization keeps the same Attempt edits and baseline.
        let reclaimed = Workspace::materialize(&assignment, &first_root, &credentials).await?;
        assert!(reclaimed.path.join("attempt-a.txt").is_file());

        let second_root = temp.path().join("attempt-b");
        tokio::fs::create_dir(&second_root).await?;
        let second = Workspace::materialize(&assignment, &second_root, &credentials).await?;
        assert!(!second.path.join("attempt-a.txt").exists());
        assert!(
            String::from_utf8(git_fixture_command(&second.path, &["status", "--short"])?)?
                .is_empty()
        );
        assert!(
            String::from_utf8(git_fixture_command(&source, &["status", "--short"])?)?.is_empty()
        );

        // A changed HEAD is not silently accepted on restart.
        git_fixture_command(&first.path, &["add", "attempt-a.txt"])?;
        git_fixture_command(&first.path, &["config", "user.name", "fixture"])?;
        git_fixture_command(
            &first.path,
            &["config", "user.email", "fixture@example.invalid"],
        )?;
        git_fixture_command(&first.path, &["commit", "-m", "unexpected"])?;
        assert!(
            Workspace::materialize(&assignment, &first_root, &credentials)
                .await
                .is_err()
        );
        Ok(())
